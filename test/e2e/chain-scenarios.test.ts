...
test('chain: create workbook → set cells in sequence → read all back', async () => {
    ...
    for (let i = 0; i < values.length; i++) {
        await set.cb({ ref: `A${i + 1}`, value: values[i] }, ctx);
    }

    for (let i = 0; i < values.length; i++) {
        const result = await get.cb({ ref: `A${i + 1}` }, ctx);
...
test('chain: create sheet → switch → write → switch back → write → verify both', async () => {
    ...
    await mockServer.getTool('select_sheet').cb({ name: 'Second' }, ctx);
    result = await mockServer.getTool('get_cell').cb({ ref: 'B1' }, ctx);
    assert.equal(result.structuredContent.value, 'sheet2-value');

    await mockServer.getTool('select_sheet').cb({ name: 'Third' }, ctx);
    result = await mockServer.getTool('get_cell').cb({ ref: 'C1' }, ctx);
...
test('chain: write cell → bold → font → background → alignment → border', async () => {
    ...
    await mockServer.getTool('set_cell').cb({ ref: 'A1', value: 'Fully Chained' }, ctx);
...
test('chain: create → write → copy sheet → verify data in both', async () => {
    ...
    await mockServer.getTool('set_cell').cb({ ref: 'A1', value: 'original-data' }, ctx);
    await mockServer.getTool('set_cell').cb({ ref: 'B1', value: 123 }, ctx);
    ...
    let r = await mockServer.getTool('get_cell').cb({ ref: 'A1' }, ctx);
    assert.equal(r.structuredContent.value, 'original-data');
    r = await mockServer.getTool('get_cell').cb({ ref: 'B1' }, ctx);
    assert.equal(r.structuredContent.value, 123);

    await mockServer.getTool('select_sheet').cb({ name: 'Sheet1' }, ctx);
    r = await mockServer.getTool('get_cell').cb({ ref: 'A1' }, ctx);
    assert.equal(r.structuredContent.value, 'original-data');
    });
});
...
test('chain: cursor moves while writing → verify each position', async () => {
    ...
    await mockServer.getTool('set_cell').cb({ ref: 'A1', value: 'pos-A1' }, ctx);
    await mockServer.getTool('move_cell_cursor').cb({ moves: [{ direction: 'right', count: 1 }] }, ctx);

    await mockServer.getTool('set_cell').cb({ ref: 'B1', value: 'pos-B1' }, ctx);
    await mockServer.getTool('move_cell_cursor').cb({ moves: [{ direction: 'down', count: 1 }] }, ctx);

    await mockServer.getTool('set_cell').cb({ ref: 'B2', value: 'pos-B2' }, ctx);
    ...
    const b2 = await mockServer.getTool('get_cell').cb({ ref: 'B2' }, ctx);
...
test('chain: rename sheet → verify data persists under new name', async () => {
    ...
    await mockServer.getTool('set_cell').cb({ ref: 'A1', value: 'before-rename' }, ctx);

    await mockServer.getTool('create_sheet').cb({ name: 'Temp' }, ctx);
    await mockServer.getTool('select_sheet').cb({ name: 'Temp' }, ctx);
    await mockServer.getTool('set_cell').cb({ ref: 'A1', value: 'temp-data' }, ctx);
    ...
    await mockServer.getTool('select_sheet').cb({ name: 'Sheet1' }, ctx);
    const sheet1 = await mockServer.getTool('get_cell').cb({ ref: 'A1' }, ctx);
...
test('chain: delete sheet → verify remaining sheets unaffected', async () => {
    ...
    await mockServer.getTool('set_cell').cb({ ref: 'A1', value: 'keep-me' }, ctx);

    await mockServer.getTool('create_sheet').cb({ name: 'Doomed' }, ctx);
    await mockServer.getTool('select_sheet').cb({ name: 'Doomed' }, ctx);
    await mockServer.getTool('set_cell').cb({ ref: 'A1', value: 'delete-me' }, ctx);
...
test('chain: rapid set_cells + search + read', async () => {
    ...
    const values: any[][] = [];
    for (let i = 0; i < 15; i++) {
        values.push([i % 3 === 0 ? `match-${i}` : `other-${i}`]);
    }
    await mockServer.getTool('set_cells').cb({ range: 'A1:A15', values }, ctx);

    const searchResult = await mockServer.getTool('search_cells').cb({ query: 'match-' }, ctx);
    assert.ok(searchResult.structuredContent);
    assert.ok(searchResult.structuredContent.matchCount >= 5);
    });
});
