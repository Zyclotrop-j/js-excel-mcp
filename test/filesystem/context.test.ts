...
const TEST_USER_OTHER = TEST_USER + '-other';
...
function cleanupOther() {
    deleteDb(TEST_USER_OTHER);
}

async function withContext<T>(userid: string, fn: (ctx: Context) => Promise<T>): Promise<T> {
    return run(async () => {
        const ctx = await Context.getContext(userid);
        try {
            return await fn(ctx);
        } finally {
            // Release should be called by request, but we need to release here for tests
            const vfs = ctx.virtualFileSystem;
            await vfs.release();
        }
    });
}
...
test('Context', async () => {
    ...
    test('getContext returns same instance within a request', async () => {
        cleanup();
        await run(async () => {
            const ctx1 = await Context.getContext(TEST_USER);
            const ctx2 = await Context.getContext(TEST_USER);
            
            assert.strictEqual(ctx1, ctx2, 'Should return same instance within a request');
    ...
    test('getContext returns different instances for different users', async () => {
        ...
        await run(async () => {
            const ctx1 = await Context.getContext(TEST_USER);
            const ctx2 = await Context.getContext(TEST_USER_OTHER);
    ...
    test('setCurrentFile and getCurrentFile round-trip', async () => {
        cleanup();
        await withContext(TEST_USER, async (ctx) => {
